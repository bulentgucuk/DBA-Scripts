...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_WhoIsActive_WithBlocking]') AND type IN (N'U'))
DROP PROCEDURE [dbo].[sp_WhoIsActive_WithBlocking]
...
License: 
	...
	written consent.
*********************************************************************************************/
CREATE PROC [dbo].[sp_WhoIsActive_WithBlocking]
(
--~
	...
	@output_column_list VARCHAR(8000) = '[dd%][session_id][block%][sql_text][sql_command][login_name][wait_info][tasks][tran_log%][cpu%][temp%][reads%][writes%][context%][physical%][query_plan][locks][%]',
...
ASE
	...
	(
		...
		UNIQUE NONCLUSTERED (transaction_id, session_id, request_id, recur
sion) WITH (IGNORE_DUP_KEY = ON)
	);
												...
												N''('' +
													CASE
														...
														WHEN
															...
															CONVERT(INT, RIGHT(y.resource_description, CHARINDEX(N'':'', REVERSE(y.resource_descript
ion)) - 1)) % 511239 = 0 
																THEN 
																	N''BCM''
														ELSE 
							...
							'index_name = ' +
								...
								'( ' +
									...
									'REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE( ' +
										...
										'NCHAR(31),N''?''),NCHAR(30),N''?''),NCHAR(29),N''?''),NCHAR(28)
,N''?''),NCHAR(27),N''?''),NCHAR(26),N''?''),NCHAR(25),N''?''),NCHAR(24),N''?''),NCHAR(23),N''?''),NCHAR(22),N''?''), ' +
										'NCHAR(21),N''?''),NCHAR(20),N''?''),NCHAR(19),N''?''),NCHAR(18),N''?''),NCHAR(17),N''?''),NCHAR(16),N''?''),NCHAR(15),N''?''),NCHAR(14),N''?''),NCHAR(12),N''?''), ' +
										'NCHAR(11),N''?''),NCHAR(8),N''?''),NCHAR(7),N''?''),NCHAR(6),N''?''),NCHAR(5),N''?''),NCHAR(4),N''?''),NCHAR(3),N''?''),NCHAR(2),N''?''),NCHAR(1),N''?''), ' +
									'NCHAR(0), ' +
									N''''' ' +
							...
							SELECT
								...
								(
									...
									WHERE 
										...
										AND COALESCE(l3.object_name, '(null)') = COALESCE(l4.object
_name, '(null)')
						...
						query_error = 
							...
							(
								...
								REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(
									...
									NCHAR(31),N'?'),NCHAR(30),N'?'),NCHAR(29),N'?'),NCHAR(28),N'?'),NCHAR(27),N'?'),NCHAR(26),N'?'),NCHAR(25),N'?'),NCHAR(24),N'?'),NCHAR(23)
,N'?'),NCHAR(22),N'?'),
									NCHAR(21),N'?'),NCHAR(20),N'?'),NCHAR(19),N'?'),NCHAR(18),N'?'),NCHAR(17),N'?'),NCHAR(16),N'?'),NCHAR(15),N'?'),NCHAR(14),N'?'),NCHAR(12),N'?'),
									NCHAR(11),N'?'),NCHAR(8),N'?'),NCHAR(7),N'?'),NCHAR(6),N'?'),NCHAR(5),N'?'),NCHAR(4),N'?'),NCHAR(3),N'?'),NCHAR(2),N'?'),NCHAR(1),N'?'),
								NCHAR(0),
				...
				'SELECT ' +
					...
					CASE 
						WHEN  @format_output IN (1, 2) THEN 
							'RIGHT ' +
							'( ' +
								'''00'' + CONVERT(VARCHAR, avg_elapsed_
time / 86400000), ' +
								'2 ' +
							') + ' +
								'RIGHT ' +
							...
							'CASE ' +
								'WHEN ' +
									...
									'AND num_events = 2 ' +
										'THEN ' +
										CASE @format_output
											WHEN 1 THEN 'CONVERT(VARCHAR, SPACE(MAX(LEN(CONVERT(VARCHAR, tempdb_current_delta))) OVER() - LEN(CONVERT(VARCHAR, tempdb_current_delta))) + LEFT(CONVERT(CHAR(22), CONVERT(MONEY, tempdb_current_delta), 1), 1
9)) '
					...
					'SELECT TOP(2147483647) ' +
						...
						CASE
							WHEN @output_column_list LIKE '%|_delta|]%' ESCAPE '|' THEN
								...
								'MAX(tempdb_current * recursion) OVER (PARTITION BY session_id, request
_id) + ' +
