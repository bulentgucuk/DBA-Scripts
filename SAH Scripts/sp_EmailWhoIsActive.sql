...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_EmailWhoIsActive]') AND type IN (N'U'))
DROP PROCEDURE [dbo].[sp_EmailWhoIsActive]
...
License: 
	...
	written consent.
*********************************************************************************************/
CREATE PROC [dbo].[sp_EmailWhoIsActive]
(
--~
	...
	@output_column_list VARCHAR(8000) = '[dd%][session_id][wait_info][sql_text][sql_command][host_name][sql_command]',
...
Non-Formatted:	[reads] [bigint] NULL
	For an active request, number of read
s done for the current query
...
10
	...
	IF @return_schema = 0
	BEGIN;
		--Disable unnecessary autostats on the tabl
e
					...
					'( ' +
						...
						'( ' +
							...
							'GROUP BY ' +
								...
								'COALESCE(NULLIF(sp2.blocked, sp2.s
pid), 0) ' +
						') AS sp1 ' +
					') AS sp0 ' +
					'WHERE ' +
												...
												'N''('' + ' +
													'CASE ' +
														...
														'WHEN ' +
															...
															'CONVERT(INT, RIGHT(y.resource_description, CHARINDEX(N'':'', REVERSE(y.resource_description)) - 1)) % 5112
33 = 0 THEN N''SGAM'' ' +
														'WHEN ' +
											...
											'( ' +
												...
												'( ' +
													'SELECT DISTINCT TOP(@i) ' +
														'wt.wait_type + ' +
															--TODO: What else can be p
ulled from the resource_description?
															'CASE ' +
		...
		BEGIN;	
			...
			(
				SELECT TOP(@i)
					...
					COALESCE(trans_nodes.trans_node.value('(request_id/text())[1]', 'INT')
, 0) AS request_id,
					...
					SET
						...
						s.statement_end_offset = 
							CASE 
								...
								WHEN SUBSTRING(CONVERT(VARCHA
R(MAX), est.text), (@statement_start_offset/2), 2) LIKE '[a-zA-Z0-9][a-zA-Z0-9]' THEN -1
						...
						'WHEN ' +
							...
							'AND physical_reads_delta >= 0 ' +
								...
								CASE @format_output
									...
									WHEN 2 THEN 'CONVE
RT(VARCHAR, LEFT(CONVERT(CHAR(22), CONVERT(MONEY, physical_reads_delta), 1), 19)) '
					...
					END + 'open_tran_count, ' +
					--sql_command
					'REPLACE(REPLACE(CONVERT(NVARCHAR(MAX), sql_command), ''<?query --''+CHAR(13)+CHAR(10), ''''), CHAR(13)+CHAR(10)+''--?>'', '''') AS '
					+ 'sql_command, ' +
					--sql_text
					'REPLACE(REPLACE(CONVERT(NVARCHAR(MAX), sql_text), ''<?query --''+CHAR(13)+CHAR(10), ''''), CHAR(13)+CHAR(10)+''--?>'', '''') AS '
					+ 'sql_text, ' +
					'query_plan, ' +
					'blocking_session_id, ' +
...
time) / 86400 ' +
	...
	Create Table #CurrentWhoIsActive (runTime varchar(50), sessionid int, wait_info varchar(100), sql_text NVARCHAR(MAX), sql_command NVARCHAR(MAX), hostname varchar(50))
	--declare @command varchar(max)
	--set @command = 'EXEC sp_executesql		'+ @sql_n + ',		N''@schema VARCHAR(MAX) OUTPUT'',		@schema OUTPUT;'
	--Print @command
	insert into #CurrentWhoIsActive
	exec (@sql_n)
	
	DECLARE @tableHTML  NVARCHAR(MAX) ;

SET @tableHTML =
    N'<H1>Current Who Is Active</H1>' +
    N'<table border="1">' +
    N'<tr><th>runtime</th>' +
    --N'<th>sessionid</th>
    N'<th>hostname</th>' +
    N'<th>sql_command</th>' +
    N'<th>wait_info</th></tr>' +
    --N'<th>sql_text</th>
    
   
    CAST ( ( SELECT td = runtime,       '',
                    --td = sessionid, '',
                    td = hostname, '',
                    td = left(isnull(sql_command, ''),25), '',
                   -- td = left(isnull(sql_text, ''), 25), '',
                    td = isnull(wait_info, '')
              FROM #CurrentWhoIsActive
              Where wait_info <> ''
...
EXEC msdb.dbo.sp_send_dbmail @recipients='Infrastructure@shopathome.com',
    @subject = 'Current Who Is Active',
    @body = @tableHTML,
    @body_format = 'HTML' ;
    Drop Table #CurrentWhoIsActive
