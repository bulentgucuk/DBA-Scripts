...
Param(
    ...
    { 
		...
		$SqlCmdAuth = ' -d "' + $DatabaseName.Replace('"', '""') + '" ' + '-G -U "' + $DatabaseUserName.Replace('"', '""') + '" ' ; 
	...
	if ($UseWindowsAuth -eq $false -and $UseAADCredential -eq $false)
		{
		$SqlCmd = '"' + $SqlCmd + '"'+ ' -b -l 60 -S "'+$DatabaseServer+ '"  -d "'+$DatabaseName+ '" -o "'+$OutputFilePath +'" -v ' + $SqlCmdVarArguments + ' -i "'+$SQLPackageFilePath+'" '
		}
	else
		{
		$SqlCmd = '"' + $SqlCmd + '"'+ ' -b -l 60 -S "'+$DatabaseServer+ '"  -o "'+$OutputFilePath +'" -v ' + $SqlCmdVarArguments + ' -i "'+$SQLPackageFilePath+'" '
		}
