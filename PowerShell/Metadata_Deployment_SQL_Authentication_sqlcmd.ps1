...
$result = Build-SQLChangeAutomationProject `
                -WorkspaceLocation "$WorkspaceLocation\New" `
                -ProjectFolder 'SSB.DB.SSBMetadata'`
                -ProjectName 'SSB.DB.SSBMetadata.sln'`
                -DeployOnBuild $false `
                -TargetServer 'vm-devops-01.ssbinfo.com'`
                -TargetDatabase 'SSBRPTest'`
...
if ($result -eq $true)
{
   ...
   Deploy-SQLChangeAutomationProject `
        -DatabaseServer 'ssb-ssb-server.database.windows.net' `
        -DatabaseName 'SSBRPTest' `
        -ProjectReleaseFolder "$WorkspaceLocation\New\SSB.DB.SSBMetadata\bin\Release" `
        -ProjectDeploymentFile 'SSB.DB.SSBMetadata_Package.sql' `
