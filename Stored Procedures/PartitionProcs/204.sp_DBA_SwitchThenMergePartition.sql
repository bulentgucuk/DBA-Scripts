...
ALTER PROCEDURE dbo.sp_DBA_SwitchThenMergePartition
	...
	, @Debug BIT = 1
	, @DryRun BIT = 1
	, @NumberOfPastPartitionsToKeep SMALLINT = 5
AS
BEGIN
	SET NOCOUNT ON;
		...
		BEGIN
			...
			IF ISDATE(@SwithBoundary) = 1
				BEGIN
					...
					SELECT @SQL = 'ALTER PARTITION FUNCTION ' + @PFName + '() MERGE RANGE (' + '''' + CONVERT(VARCHAR(10), @SwithBoundaryDate, 111) + '''' + ');' ;
