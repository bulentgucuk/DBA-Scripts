...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[pr_DatabaseBackup]') AND type in (N'P', N'PC'))
    BEGIN
        PRINT 'Dropping procedure [pr_DatabaseBackup] - SQL 2008'
        DROP PROCEDURE [dbo].[pr_DatabaseBackup]
    END
GO

/****** Object:  StoredProcedure [dbo].[pr_DatabaseBackup]    Script Date: 09/23/2012 10:40:57 ******/
SET ANSI_NULLS ON
GO
...
SET NOCOUNT ON
    ...
    SELECT @VerifyBase = 'RESTORE VERIFYONLY FROM DISK=''' + @BackupFolder + '~sf~' + '~d~' + @TimeStamp + '.~s~'' '
