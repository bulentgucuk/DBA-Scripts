...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[OpenSPBack0050_RepToBP15d]') AND type in (N'P', N'PC'))
BEGIN
    EXEC( 'CREATE PROCEDURE [dbo].[OpenSPBack0050_RepToBP15d] AS' )
END
GO


ALTER PROCEDURE dbo.OpenSPBack0050_RepToBP15d ( 
    @Status int =0 OUTPUT)
...
EXEC dbadmin.dbo.OpenSPBack0050_RepToBP15d @status=@status OUTPUT
END
IF (SELECT @status ) <> 0 
 BEGIN
 ...
 EXECUTE dbadmin.dbo.[pr_DatabaseBackup]
        ...
        @BackupFolder = '\\SPBACK0050.web.prod\RepToBP15d\SERVERNAME',     -- REQUIRED. Target Folder, should NOT include DB specific folder - use @CreateSubFolder.
...
--Command to execute
SELECT @cmdline ='Net use \\SPBACK0050.web.prod\RepToBP15d Fr0Ntr@nG3!! /USER:web.prod\svc_SQL_DD /PERSISTENT:YES';
