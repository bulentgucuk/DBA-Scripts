/***
SELECT [compatibility_level], db_name() FROM sys.databases WHERE database_id = DB_ID()
...
exec sp_helptext 'dbo.sp_WhoIsActive'
...
WHERE	StartTime > '20190322'
--AND		CommandType = 'ALTER_INDEX'
ORDER BY ID DESC;


*****/
--drop USER ssb_azure_dba 
--Check objecs needed for Azure SQL DB Index maintenance
select * FROM sys.objects WHERE NAME IN ('commandlog', 'commandexecute', 'indexoptimize')

IF NOT EXISTS(SELECT * FROM SYS.DATABASE_PRINCIPALS WHERE NAME = 'ssb_bgucuk')
BEGIN
	CREATE USER ssb_bgucuk           WITH PASSWORD = 'YourSecurePassword';
END
GO
ALTER ROLE db_owner         ADD MEMBER [ssb_bgucuk];
GO

-- Crete the SQL User for Azure Automation
IF NOT EXISTS(SELECT * FROM SYS.DATABASE_PRINCIPALS WHERE NAME = 'ssb_azure_dba')
BEGIN
	CREATE USER ssb_azure_dba           WITH PASSWORD = 'jC73dYDu82BmwYNyTq';
END
GO
ALTER ROLE db_owner         ADD MEMBER [ssb_azure_dba];
GO

-- Below here is one table and two stored procedures from Ola Hollangren
SET ANSI_NULLS ON
GO
